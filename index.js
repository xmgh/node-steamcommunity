...
SteamCommunity.prototype.login = function(details, callback) {
	if (!details.accountName || !details.password) {
		throw new Error(
			"Missing either accountName or password to login; both are needed"
		);
	}
	...
	if (!disableMobile) {
		mobileHeaders = {
			...
			"User-Agent":
				this._options.mobileUserAgent ||
				details.mobileUserAgent ||
	...
	this.httpRequestPost(
		...
		function(err, response, body) {
			...
			if (!disableMobile) {
				...
				formObj.loginfriendlyname =
					"#login_emailauth_friendlyname_mobile";
			}
			...
			self.httpRequestPost(
				...
				function(err, response, body) {
					...
					if (!body.success && body.emailauth_needed) {
						...
						!body.success &&
						body.captcha_needed &&
						body.message.match(/Please verify your humanity/)
					) {
						...
						callback(
							...
							cookies,
							steamguard,
							disableMobile ? null : oAuth.oauth_token
						);
					}
				},
				"steamcommunity"
	...
	this.httpRequestPost(
		...
		function(err, response, body) {
			...
			var cookies = [
				"steamLogin=" +
					encodeURIComponent(
						steamID.getSteamID64() + "||" + body.response.token
					),
				"steamLoginSecure=" +
					encodeURIComponent(
...
SteamCommunity.prototype.setCookies = function(cookies) {
	cookies.forEach(cookie => {
		...
		this._setCookie(
			...
			!!(
				cookieName.match(/^steamMachineAuth/) ||
				cookieName.match(/Secure$/)
			)
		);
	});
};

SteamCommunity.prototype.getCookie = function(name) {
	var cookies = this._jar
		.getCookieString("http://steamcommunity.com")
		.split(";");
	for (var i = 0; i < cookies.length; i++) {
...
SteamCommunity.prototype.loggedIn = function(callback) {
	this.httpRequestGet(
		...
		function(err, response, body) {
			if (
				err ||
				(response.statusCode != 302 && response.statusCode != 403)
			) {
	...
	if (this._profileURL) {
		...
		this.httpRequest(
			...
			function(err, response, body) {
				...
				var match = response.headers.location.match(
					/steamcommunity\.com(\/(id|profiles)\/[^\/]+)\/?/
				);
				if (!match) {
